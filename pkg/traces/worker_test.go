...
func TestSpanStatuses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("inputStatus="+tt.inputStatus, func(t *testing.T) {
			syncer := &mockSyncer{}
...
func TestConfigHelpers(t *testing.T) {
	...
	cfg := NewConfig()
	assert.NotNil(t, cfg)
	assert.Equal(t, 1, cfg.NumTraces)
	assert.Equal(t, 1, cfg.NumChildSpans)
	assert.Equal(t, "/v1/traces", cfg.HTTPPath)
	assert.False(t, cfg.PropagateContext)
	assert.True(t, cfg.Batch)
	assert.Equal(t, 0, cfg.LoadSize)
	assert.Equal(t, 123*time.Microsecond, cfg.SpanDuration)
